...
def epoch_general_cifar10(dataloader, model, epoch, loss_fn=nn.SoftmaxLoss(), opt=None):
    ...
    batch_device = params[0].device if params else device
    for X, y in dataloader:
        X = ndl.Tensor(X, device=batch_device, dtype="float32")
        y = ndl.Tensor(y, device=batch_device, dtype="float32", requires_grad=False)
        logits = model(X)
