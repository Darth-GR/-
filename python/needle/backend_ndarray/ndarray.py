...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        assert self.is_compact(), "Expect NDArray to be compact"
    ...
    def __getitem__(self, idxs):
        ...
        shape = tuple(math.ceil((sl.stop - sl.start) / sl.step) for sl in idxs)
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert isinstance(axes, tuple) and len(axes) > 0, "Invalid axes"

        dim_offsets = [0 for _ in self.shape]

        dim_prod = 1
        for i in reversed(range(len(self.shape))):
            dim_offsets[i] = (self.shape[i] - 1) * dim_prod
            dim_prod *= self.shape[i]


        offset = 0
        for axis in axes:
            offset += dim_offsets[axis]

        strides = tuple([s if i not in axes else -s for i, s in enumerate(self.strides)])
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert len(axes) == self.ndim, "Mismatched number of dimensions"
        assert all(len(axis) == 2 for axis in axes), "Must be tuple of size-2 tuple"

        new_shape = tuple([dim + axes[i][0] + axes[i][1] for i, dim in enumerate(self.shape)])
        array = NDArray.make(new_shape, device=self._device)
        idxs = tuple([slice(l, new_shape[i] - r) for i, (l, r) in enumerate(axes)])
